...
class TCControllerIntegrationTest {
  ...
  @Test
  void getTcsByModule_withOwnedModule_returnsSummaries() throws Exception {
    User user = userRepository.save(new User("Tauzih", "tauzih@example.com", "hashed"));
    CourseModule module = saveModule(user, "Trees", "Graphs");

    saveTcWithEntries(module, user, "Trees", 1);
    saveTcWithEntries(module, user, "Graphs", 1);

    mockMvc.perform(get("/api/v1/modules/" + module.getId() + "/tcs")
  ...
  @Test
  void getTcsByModule_marksStaleSummaryWhenTopicWasRemovedFromModule() throws Exception {
    User user = userRepository.save(new User("Tauzih", "tauzih@example.com", "hashed"));
  ...
  @Test
  void getTcsByModule_withOtherUsersModule_returnsNotFound() throws Exception {
    ...
    saveTcWithEntries(module, owner, "Trees", 1);
  ...
  @Test
  void getTcsByModule_withoutAuthentication_isForbidden() throws Exception {
    mockMvc.perform(get("/api/v1/modules/12/tcs"))
        .andExpect(status().isForbidden());
  }

  @Test
  void getTcById_withOwnedTc_returnsDetailWithEntriesNewestFirst() throws Exception {
    User user = userRepository.save(new User("Tauzih", "tauzih@example.com", "hashed"));
    CourseModule module = saveModule(user, "Trees");
    TC tc = saveTcWithEntries(module, user, "Trees", 2);

    mockMvc.perform(get("/api/v1/tcs/" + tc.getId())
        .with(authentication(authFor(user))))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(tc.getId()))
  ...
  @Test
  void getTcById_withStaleTc_stillReturnsReadableDetail() throws Exception {
    User user = userRepository.save(new User("Tauzih", "tauzih@example.com", "hashed"));
    CourseModule module = saveModule(user, "Trees");
  ...
  @Test
  void getTcById_withOtherUsersTc_returnsNotFound() throws Exception {
    User owner = userRepository.save(new User("Tauzih", "tauzih@example.com", "hashed"));
    User otherUser = userRepository.save(new User("Dhruv", "dhruv@example.com", "hashed"));
    CourseModule module = saveModule(owner, "Trees");
  ...
  @Test
  void getTcById_withoutAuthentication_isForbidden() throws Exception {
    mockMvc.perform(get("/api/v1/tcs/99"))
        .andExpect(status().isForbidden());
  }

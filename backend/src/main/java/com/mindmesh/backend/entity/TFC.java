...
public class TFC {
  ...
  // Source of truth for a TFC is (module (+ owner since module already has
  ...
  public TFC(CourseModule module, User owner, String topic) {
    this.module = module;
    this.owner = owner;
    this.topic = topic;
  }
  ...
  public void addEntry(CFCEntry entry) {
    ...
    if (entry.getTfc() != null && entry.getTfc() != this) {
      entry.getTfc().removeEntry(entry);
    }
  ...
  public void removeEntry(CFCEntry entry) {
    ...
    if (entries.remove(entry) && entry.getTfc() == this) {
      entry.setTfc(null);
    }
  }
